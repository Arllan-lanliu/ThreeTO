...
def _forward_scores(
    ...
    if strategy not in {"oracle", "total", "vote"}:
        raise ValueError(
            f"Unknown eval_strategy {strategy!r}; allowed: oracle, total, vote"
        )
    ...
    with torch.no_grad():
        for feat, _, labels, class_types, _ in tqdm(
            loader, leave=False, desc=f"{strategy}_scores"
        ):
...
def train(args: argparse.Namespace) -> torch.nn.Module:
    ...
    eval_strat = str(getattr(args, "eval_strategy", "oracle")).strip().lower()
    if eval_strat not in {"oracle", "total", "vote"}:
        raise ValueError(
            f"Unknown eval_strategy {eval_strat!r}; allowed: oracle, total, vote"
        )
    ...
    for epoch in tqdm(range(epoch0, int(args.num_epochs)), desc="epochs"):
        ...
        for i, (feat, _, labels, class_types, _) in enumerate(
            ...
            sco_s, lab_s, _ = _forward_scores(model, sample_ld, args, eval_strat)
            eer_s, f1_s, thr_s = _scores_to_metrics(
                sco_s, lab_s, args.eval_threshold_mode, thr_fix
            )
            mv = chk_metric(mean_dev_loss, eer_s, f1_s)
            ...
            with open(Path(args.log_dir, "dev_loss.log"), "a", encoding="utf-8") as lf:
                lf.write(
                    f"{gs}\tsample_{eval_strat}\teer:{eer_s:.6f}\tf1:{f1_s:.6f}"
                    f"\tthr:{thr_s:.4f}\tmult_ce:{mean_dev_loss:.6f}\n"
                )
            ...
            if use_wandb:
                wandb.log(
                    {
                        f"sample/{eval_strat}_eer": eer_s,
                        f"sample/{eval_strat}_f1": f1_s,
            ...
            if (
                ...
                sco_f, lab_f, type_f = _forward_scores(
                    model, full_ld, args, eval_strat
                )
                selected_metrics = _scores_to_detailed_metrics(
                    sco_f, lab_f, type_f, args.eval_threshold_mode, thr_fix
                )
                ml_full = _mean_dev_loss(model, full_ld, args, cw)
                chk_f = chk_metric(
                    ml_full,
                    float(selected_metrics["eer"]),
                ...
                full_row = {
                    ...
                    **_flatten_full_metrics(eval_strat, selected_metrics),
                }
                ...
                # Log only the configured strategy to avoid duplicate full-dev passes.
                with open(Path(args.log_dir, "dev_loss.log"), "a") as lf:
                    lf.write(
                        f"{gs}\tfull_{eval_strat}\teer:{selected_metrics['eer']:.6f}"
                        f"\tf1:{selected_metrics['macro_f1']:.6f}"
                        f"\tacc:{selected_metrics['accuracy']:.6f}"
                        f"\tthr:{selected_metrics['threshold']:.4f}"
                        f"\tmult_ce:{ml_full:.6f}\n"
                    )
                    for t_name, tm in selected_metrics["per_type"].items():
                        lf.write(
                            f"{gs}\tfull_{eval_strat}_{t_name}"
                ...
                if use_wandb:
                    wandb.log(
                        {
                            f"full/{eval_strat}_eer": selected_metrics["eer"],
                            f"full/{eval_strat}_f1": selected_metrics["macro_f1"],
