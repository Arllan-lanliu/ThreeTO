...
class CQCCExtractor(nn.Module):
    ...
    def _forward_torch(self, audio: torch.Tensor) -> torch.Tensor:
        """GPU-friendly CQCC-like frontend.

        This keeps the same CQCC output contract while avoiding the CPU-only
        librosa CQT path. It uses an STFT power spectrum projected onto the
        configured logarithmic frequency bins, followed by DCT and deltas.
        """
        x = audio.float()
        ...
        power = spec.abs().square().transpose(1, 2)  # (B, T, F)
    ...
    def _logfreq_filterbank(
        ...
        centers = self.fmin * (2.0 ** (torch.arange(self.n_bins, device=device, dtype=dtype) / self.bins_per_octave))
        ...
        left = (freqs.unsqueeze(0) - lower.unsqueeze(1)) / (centers.unsqueeze(1) - lower.unsqueeze(1)).clamp_min(1e-6)
        right = (upper.unsqueeze(1) - freqs.unsqueeze(0)) / (upper.unsqueeze(1) - centers.unsqueeze(1)).clamp_min(1e-6)
